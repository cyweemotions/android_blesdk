 ...
 * 睡眠监测算法设置
 */
public class SleepMonitorTask extends OrderTask {
    private byte[] orderData;
    public SleepMonitorTask (MokoOrderTaskCallback callback, SleepMonitor sleepMonitor) {
        super(OrderType.WRITE, OrderEnum.setSleepMonitor, callback, OrderTask.RESPONSE_TYPE_WRITE_NO_RESPONSE);
        List<Byte> dataList = new ArrayList<>();
        dataList.add((byte) (sleepMonitor.heighPrecisionDetec & 0xFF));// 睡眠高精度检测 0-开 1-关
        dataList.add((byte) (sleepMonitor.breatheQualityDetec & 0xFF));// 睡眠呼吸质量检测 0-开 1-关
